...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        # 2.2 I create the result variable
        res = 0

        # 2.3 I define the variables for a better understanding
        name = stations[1]
        status = int(stations[0])
        bikes_available = int(stations[5])
        dateandtime = stations[4]
        # 2.3.1 Split the date and time into separate variables
        params = dateandtime.split(" ")
        date = params[0]
        time = params[1]
        # 2.3.2 Obtain the actual name of the specific date
        weekday = get_day_of_week(date)

        # 2.4 If the station is the one from the input parameters and also it satisfies the condition I add 1 to the
        # output variable
        if str(name) == str(input_name):
            if status == 0 and bikes_available == 0:
                res = res + 1

        # 2.5 Write the res variable into the file with its corresponding name (day_hour)
        if res > 0:
            msg = str(weekday) + "_" + getNumHour(time) + "\t(" + str(res) + ")\n"
